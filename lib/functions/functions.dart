...
//base url
String url = 'https://monitorride.com/'; //add '/' at the end of url as 'yourwebsite.com/'
...
validateEmail(email) async {
  ...
  try {
    print(email);
    print(userDetails['role'].toString());
    print(ischeckownerordriver);
  ...
  Timer.periodic(const Duration(seconds: 5), (timer) async {
    if (userDetails.isNotEmpty && userDetails['role'] == 'driver') {
      ...
      if (userDetails['role'] == 'driver') {
        ...
        if (_driverStatus.child('is_deleted').value == 1) {
          FirebaseDatabase.instance
              .ref()
              .child('drivers/' + userDetails['id'].toString())
              .remove();
...
userLogout() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      // print(id);
      final _position = FirebaseDatabase.instance.ref();
      _position.child('drivers/' + id.toString()).update({
        'is_active': 0,
      });
